# Airflow DAG to stop a set of EC2 instances on a schedule
...
with DAG(
    Path(__file__).stem,
    schedule="00 18 * * *",
    description="Lookup EC2 instances by tag and stop them on schedule",
    default_args={**default_args, "email": Variable.get("alert_email")},
    catchup=False,
    tags=["it-operations"],
):
    stop_instance = EC2StopInstanceOperator.partial(task_id="stop_instance").expand(
